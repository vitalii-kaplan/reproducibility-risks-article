...
study, combining OpenAlex bibliometrics, an expanded top-cited article audit
registry,
...
rather than downloadable workflow artifacts. In the 80-record expanded audit
registry, twenty-two records report downloadable or linked KNIME workflow
files. We
...
Our top-cited article collection now covers the original 60 most-cited
KNIME-matching records and citation ranks 61--80, with not-assessed
placeholders retained where local full text is unavailable. The completed
structured audit subset reported in
Table~\ref{tab:top-cited-assessment} shows that publication practice is the
weak link. In the 80-record expanded audit registry, twenty-two records
report downloadable or linked KNIME workflow files. Current retrieval and manual
...
OpenAlex records by \texttt{cited\_by\_count} in descending order and first
retained the 60 most-cited records in
\path{data/processed/openalex/openalex_knime_most_cited.csv}. We then expanded
the registry to citation ranks 61--80, adding detailed assessments for records
with local full text and not-assessed placeholders for records still missing
local full text. Open-access papers were downloaded directly, and
non-open papers were requested through the author's institutional subscription
access or added manually when available. At the time of this draft, the
expanded structured audit contains 80 article records: the original top-60
registry plus all 20 records from ranks 61--80. Fifteen of the rank 61--80
records have local PDF files and processed one-column text; five are retained
as not-assessed placeholders. In total, 16 records are retained as
not-assessed placeholders because no local full text is available. Downloaded
papers receive detailed fields and quote-supported flags, while records without
local full text retain not-assessed metadata.
...
The top-cited article dataset records the manual assessment of the original
top-60 OpenAlex records and citation ranks 61--80. Detailed fields are present
for records with local full text, while records still missing local full text
are retained as not-assessed placeholders.
The public replication package contains the processed assessment metadata, but
not the article PDFs. The processed assessment is stored in
\path{data/processed/audit/knime_most_cited_article_assessments.json}. The
ranked source list is available as
\path{data/processed/openalex/openalex_knime_most_cited.csv}, with rank-range
download attempts recorded under \path{data/processed/audit/}.
Each record in the assessment JSON stores the OpenAlex rank, manual assessment
status, KNIME role, reported KNIME version, workflow-artifact status, data and
code availability, linked resources visible in the paper, quote-backed support
for positive flags, and short evidence notes. Records without accessible full
text are retained in the JSON as not-assessed placeholders. This keeps the
original top-60 denominator explicit while allowing the expanded full-text
audit records to be reported separately.

The PAINS filters paper is one example in which the assessment found
reproduction-relevant workflow identifiers \cite{Saubern2011PAINS}. The
assessment records the following identifiers:
\begin{itemize}
...
We built an expanded top-cited retrieval pool from OpenAlex records matching
\texttt{KNIME}. The statistics in this subsection use the 80-record structured
audit registry: the original top-60 registry plus all 20 records from citation
ranks 61--80. The purpose of this part of the study is
not to decide whether KNIME is
...
cases. Six of the 80 records are about KNIME itself or a KNIME extension. These
...
flag maps empty. Table~\ref{tab:top-cited-assessment} uses all 80 records in
the expanded registry as the denominator. It first reports the classification
of the records, then reports the workflow-reporting flags observed in the
structured audit.

Table~\ref{tab:top-cited-assessment} summarizes the statistics-ready flags in
the structured audit file. Fifty-one records use KNIME as a workflow, tool,
interface, or implementation context. Forty-four records contain textual
workflow or node descriptions, and thirty show workflow screenshots or figures.
Twenty-two records report downloadable or linked KNIME workflow files. Current
retrieval attempts have so far obtained workflow artifacts for seven of these
article records; one additional repository was reachable but did not contain a
KNIME workflow file. Version and dependency reporting remain limited: fifteen
records report a KNIME version, twenty report KNIME extension or plugin
dependencies, and eleven report an installation source. Data and code signals
are more common than retrievable workflow-artifact evidence: forty-one records
report input-data availability, twenty provide a direct input-data resource,
and fifteen provide code or scripts.
...
\caption{Flag summary for the expanded top-cited article audit. Shares use all
80 records in the expanded registry as the denominator.}
\label{tab:top-cited-assessment}\\
\toprule
Audit category or flag & Count & Share of 80 & Interpretation \\
\midrule
\endfirsthead
\toprule
Audit category or flag & Count & Share of 80 & Interpretation \\
...
\bottomrule
\endfoot
Expanded audit records & 80 & 100.0\% & Original top-60 records plus 20 rank 61--80 records \\
Assessed from full text & 64 & 80.0\% & Article text available for manual assessment \\
Not assessed from full text & 16 & 20.0\% & Full text unavailable at assessment time \\
About KNIME & 6 & 7.5\% & Background/platform or extension papers, not deeper workflow-reporting cases \\
Not a KNIME use case & 7 & 8.8\% & KNIME appears in the record but is not a KNIME-based study \\
\midrule
Uses KNIME & 51 & 63.7\% & KNIME used as a workflow, tool, interface, or implementation context \\
Workflow or nodes described in text & 44 & 55.0\% & Named workflow steps, nodes, modules, or components recorded \\
Workflow screenshots or figures & 30 & 37.5\% & Workflow shown visually in article or supplement \\
Reports KNIME version & 15 & 18.8\% & Specific KNIME version value found \\
Downloadable KNIME workflow files & 22 & 27.5\% & Executable workflow files provided or linked \\
Reports extension/plugin dependencies & 20 & 25.0\% & KNIME extension or node-library dependency reported \\
Reports extension installation source & 11 & 13.8\% & Update site, project URL, or installation source reported \\
Linked workflow artifacts retrieved & 7 & 8.8\% & Linked KNIME workflow artifacts obtained in project notes \\
\midrule
Provides code or scripts & 15 & 18.8\% & Code repository, scripts, or software code reported \\
Reports input-data availability & 41 & 51.2\% & Data availability stated, with or without direct URL \\
Direct input-data resource & 20 & 25.0\% & Direct data URL or resource pointer recorded \\
\end{longtable}
\endgroup
...
\begin{longtable}{|%
  ...
  r|
  r|}
\caption{Workflow-reporting signals among the 51 assessed KNIME-use cases.}
...
\endfoot
Uses KNIME & 51 & 100.0\% \\
Workflow or nodes described in text & 44 & 86.3\% \\
Workflow screenshots or figures & 30 & 58.8\% \\
Reports KNIME version & 15 & 29.4\% \\
Downloadable KNIME workflow files & 22 & 43.1\% \\
Reports extension/plugin dependencies & 20 & 39.2\% \\
Reports extension installation source & 11 & 21.6\% \\
Linked workflow artifacts retrieved & 7 & 13.7\% \\
\end{longtable}
\endgroup
