...
study, combining OpenAlex bibliometrics, a manual assessment of the 40
...
rather than downloadable workflow artifacts. Among the 40 studied records,
eight report downloadable or linked KNIME workflow files. We could obtain
workflow artifacts for four article records and open five KNIME workflows from
those artifacts in a current local KNIME environment; only one workflow
...
Our manual assessment of the 40 most-cited KNIME-matching records shows that
publication practice is the weak link. Among the assessed non-KNIME-focused
records in Table~\ref{tab:top-cited-assessment}, eight report downloadable or
linked KNIME workflow files. Current retrieval and manual opening results were
mixed: workflow artifacts were obtained for four article records, five KNIME
workflows were opened, and only one workflow executed successfully in the
tested local environment. For other influential studies, the strongest
workflow evidence is usually weaker than an executable artifact. Several
records present screenshots or figures of connected KNIME nodes, or describe
workflow steps in text, without preserving all executable workflow context.
...
40 most-cited KNIME-matching articles, current-version workflow opening and
execution attempts, and repository-level mining of KNIME node metadata. The goal is to
...
\texttt{cited\_by\_count} in descending order and retained the 40 most-cited
records in
\path{data/processed/openalex/openalex_knime_most_cited.csv}.
We then attempted to retrieve all 40 articles. Open-access papers were
downloaded directly, and non-open papers were requested through the authors'
institutional subscription access. Thirty-five of the 40 selected records were
obtained and assessed from full text. Five records were not obtained through
...
only to support extraction and consistency checking from the accessible full
text. The assessment was organized as a structured audit rather than a
free-form note file. First, each record was assigned descriptive fields for
...
background or platform papers about KNIME were kept in the 40-record sample,
...
We manually audited the 40 most-cited OpenAlex records matching \texttt{KNIME}
...
cases. Five of the 40 records are about KNIME itself or a KNIME extension. These
...
flag maps empty. Table~\ref{tab:top-cited-assessment} uses all 40 top-cited
...
the structured audit file. Twenty-four records use KNIME as a workflow, tool,
interface, or implementation context. Nineteen records contain textual workflow
or node descriptions, and eleven show workflow screenshots or figures. Eight
records report downloadable or linked KNIME workflow files. Current retrieval
attempts obtained workflow artifacts for four of these article records; one
additional repository was reachable but did not contain a KNIME workflow file.
Version and dependency reporting remain limited: seven records report a KNIME
version, six report KNIME extension or plugin dependencies, and three report an
installation source. Data and code signals are more common than retrievable
workflow-artifact evidence: seventeen records report input-data availability,
nine provide a direct input-data resource, and nine provide code or scripts.
...
\caption{Flag summary for the top-cited article audit. Shares use all 40
top-cited records as the denominator.}
\label{tab:top-cited-assessment}\\
\toprule
Audit category or flag & Count & Share of 40 & Interpretation \\
\midrule
\endfirsthead
\toprule
Audit category or flag & Count & Share of 40 & Interpretation \\
...
\bottomrule
\endfoot
All top-cited records & 40 & 100.0\% & Records retained from the OpenAlex ranking \\
Assessed from full text & 35 & 87.5\% & Article text available for manual assessment \\
Not assessed from full text & 5 & 12.5\% & Full text unavailable at assessment time \\
About KNIME & 5 & 12.5\% & Background/platform or extension papers, not deeper workflow-reporting cases \\
Not a KNIME use case & 6 & 15.0\% & KNIME appears in the record but is not a KNIME-based study \\
\midrule
Uses KNIME & 24 & 60.0\% & KNIME used as a workflow, tool, interface, or implementation context \\
Workflow or nodes described in text & 19 & 47.5\% & Named workflow steps, nodes, modules, or components recorded \\
Workflow screenshots or figures & 11 & 27.5\% & Workflow shown visually in article or supplement \\
Reports KNIME version & 7 & 17.5\% & Specific KNIME version value found \\
Downloadable KNIME workflow files & 8 & 20.0\% & Executable workflow files provided or linked \\
Reports extension/plugin dependencies & 6 & 15.0\% & KNIME extension or node-library dependency reported \\
Reports extension installation source & 3 & 7.5\% & Update site, project URL, or installation source reported \\
Linked workflow artifacts retrieved & 4 & 10.0\% & Linked KNIME workflow artifacts obtained in project notes \\
\midrule
Provides code or scripts & 9 & 22.5\% & Code repository, scripts, or software code reported \\
Reports input-data availability & 17 & 42.5\% & Data availability stated, with or without direct URL \\
Direct input-data resource & 9 & 22.5\% & Direct data URL or resource pointer recorded \\
\end{longtable}
\endgroup
...
\begin{longtable}{|%
  ...
  r|
  r|}
\caption{Workflow-reporting signals among the 24 assessed KNIME-use cases.}
...
\endfoot
Uses KNIME & 24 & 100.0\% \\
Workflow or nodes described in text & 18 & 75.0\% \\
Workflow screenshots or figures & 10 & 41.7\% \\
Reports KNIME version & 5 & 20.8\% \\
Downloadable KNIME workflow files & 8 & 33.3\% \\
Reports extension/plugin dependencies & 6 & 25.0\% \\
Reports extension installation source & 3 & 12.5\% \\
Linked workflow artifacts retrieved & 4 & 16.7\% \\
\end{longtable}
\endgroup

The workflow-level experiment then tested the eight article records that
reported downloadable or linked KNIME workflow files. The purpose was limited:
...
without manual repair. Four article records yielded workflow artifacts. Because
the PAINS record contains separate RDKit and Indigo workflows, these four
article records produced five workflow-opening attempts. Only the PAINS RDKit
workflow executed successfully. The other opened workflows either failed during
execution, required missing R packages or extensions, or could not be
confidently executed from the available workflow state. Across the opened
...
eight article records with downloadable or linked KNIME workflow files.}
