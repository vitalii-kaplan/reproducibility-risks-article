...
Thirteen scripted steps are deterministic. Two scripted steps use an LLM.
We use the LLM only where rule-based extraction is insufficient. The first LLM
step classifies cached reference pages. The second LLM step fills
supplementary article-level audit flags. The LLM calls use temperature 0 to
reduce variation. Repeated execution may still depend on the model and service
version used.
