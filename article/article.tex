...
We manually assessed the 20 most-cited records in this OpenAlex result set,
using the downloaded PDFs where available. This assessment turns the
bibliometric result into a workflow-preservation question. Raw counts show that
KNIME appears in the literature, but they do not show whether authors published
workflow files, reported KNIME versions, preserved data, or described extension
dependencies. Influential KNIME matches include platform papers,
software-overview papers, tool-comparison papers, papers that use KNIME as one
analysis tool, and papers that publish KNIME workflow artifacts.

Five of the top-20 records are primarily about KNIME itself, a KNIME extension,
or a KNIME tutorial: the KNIME 2.0 platform paper, the Pipeline Pilot and KNIME
workflow-system comparison, KNIME-CDK, the 2006 KNIME platform paper, and the
KNIME bioimage-analysis tutorial. These records are useful background evidence
for KNIME's existence, architecture, and ecosystem, but they are not the main
case-study target for published scientific workflows that use KNIME as an
analysis tool. We therefore exclude them from
Table~\ref{tab:top-cited-assessment} and use the table to summarize the
remaining records assessed from full text. One additional top-20 record, the
QSAR curation procedure, is also outside the table because its full text was
not obtained through open access or through the authors' subscription access.
The table supports the study in two ways. First, it confirms that KNIME is
visible in highly cited computational studies beyond papers about the KNIME
platform itself. Second, it shows that data availability,
software availability, and workflow availability are separate properties.
Several papers provide data or code signals but do not provide a downloadable
KNIME workflow or report a KNIME version. This is exactly the setting in which
future readers cannot tell whether a failure comes from missing data, missing
extensions, changed node behavior, or deprecated nodes. For these papers, the
strongest workflow evidence is usually an image of connected KNIME nodes in the
article or supplement, not an executable workflow file. In this top-cited set,
the PAINS filters paper is the only case for which we could retrieve workflow
files and perform a current-KNIME import experiment.
In the extracted evidence, no non-KNIME-focused record reported a separate
dataset DOI. Where the paper DOI is used as a supplementary-material landing
page, we record that as a supplement signal rather than as a dataset DOI.
...
\begin{longtable}{|%
  ...
  >{\raggedright\arraybackslash}p{0.17\textwidth}|
  >{\raggedright\arraybackslash}p{0.18\textwidth}|}
\caption{Manual assessment of top-cited records excluding KNIME-focused papers.}
\label{tab:top-cited-assessment}\\
\toprule
Paper & Year & KNIME role & Version & Workflow artifact &
Data/code signal \\
\midrule
\endfirsthead
\toprule
...
\bottomrule
\endfoot
CARE image restoration
\newline\path{10.1038/s41592-018-0216-7} &
2018 &
KNIME workflows as user interface &
Not found &
Supplement image only &
Data/code URL, no workflow file found \\
Pump fault prediction
\newline\path{10.3390/su12114776} &
2020 &
KNIME analysis workflow &
Not found &
On figures / described only &
Supplier data \\
Open-source pKa QSAR
\newline\path{10.1186/s13321-019-0384-1} &
2019 &
KNIME data preparation &
Not found &
No workflow found &
GitHub code/data only \\
Engineering student outcomes
\newline\path{10.1016/j.heliyon.2019.e01250} &
2019 &
KNIME analysis workflow &
Not found &
Described only &
Supplement via paper DOI \\
ImgLib2
\newline\path{10.1093/bioinformatics/bts543} &
2012 &
KNIME ecosystem mention &
Not found &
Not applicable &
Code URL and journal supplement \\
Heart disease classification
\newline\path{10.1007/s12553-020-00438-1} &
2020 &
Tool comparison &
4.0.2 &
No workflow found &
UCI data URL \\
Latest data-mining tools
\newline\path{10.35940/ijitee.i1003.0789s19} &
2019 &
General tool review &
2.9 &
Not applicable &
...
\newline\path{10.1016/j.procs.2016.05.251} &
2016 &
Tool comparison &
Not found &
No workflow found &
...
\newline\path{10.1109/mipro.2014.6859735} &
2014 &
General tool review &
2.9.1 &
Not applicable &
...
\newline\path{10.1177/1460458219899210} &
2020 &
KNIME and R analysis &
Not found &
No workflow found &
Clinical data not public \\
PAINS filters workflow
\newline\path{10.1002/minf.201100076} &
2011 &
KNIME workflow paper &
2.3.4 &
Supporting info and myExperiment &
Data URL and supplement via paper DOI \\
First-year performance
\newline\path{10.1007/s10639-018-9839-7} &
2018 &
KNIME analysis workflow &
Not found &
On figures / described only &
Dataset source and paper DOI \\
Classification-tool study
\newline\path{10.14569/specialissue.2011.010304} &
2011 &
Tool comparison &
Not found &
No workflow found &
UCI URL and paper DOI \\
kGCN
\newline\path{10.1186/s13321-020-00435-6} &
2020 &
KNIME GUI for software &
Not found &
On figures / described only &
GitHub/Docker and paper DOI \\
\end{longtable}
\endgroup
