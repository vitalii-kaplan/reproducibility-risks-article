...
the 100-record article audit registry, twenty-three records report
downloadable or linked KNIME workflow files. Recorded retrieval attempts
obtained workflow artifacts for eighteen article records. In the recorded
manual execution checks, four article records had at least one workflow execute
successfully.
...
weak link. Twenty-three records report downloadable or linked KNIME workflow
files, and workflow artifacts were obtained for eighteen article records in
...
execution checks, four article records had at least one workflow execute
...
thirteen scripted steps are deterministic, and two use an LLM for bounded
judgment tasks where rule-based extraction is insufficient, namely
classification of cached reference pages and supplementary article-level audit
flags. The LLM calls use temperature 0 to reduce variation, but repeated
execution may still depend on the model and service version used. 

The pipeline
is mostly automatic, but it is deliberately semi-automatic at two boundary
...
\path{scripts/attach_url_fetch_metadata.py}. This deterministic step augments
...
\path{scripts/build_article_audit_report.py}. This deterministic join combines
...
\path{scripts/download_workflow_artifacts.py}. The script uses deterministic
...
\path{data/processed/audit/article_assessments.json}. Each
...
The top-cited article assessment uses the data products created by the
semi-automatic audit application described in Section~3.2. The starting data
are the citation-ranked OpenAlex seed table and the local article registry.
The first new article-level evidence is the set of local PDFs under
\path{data/original/articles/}, together with download-attempt logs under
\path{data/processed/audit/logs/}. These files record which OpenAlex seed
records have locally available article text. The next output is the GROBID
conversion result: semantic HTML files under \path{data/processed/articles/}
and TEI XML files under \path{data/processed/articles/grobid_tei/}. These
outputs provide the article text, section structure, bibliography context, and
machine-readable links used by later steps. The absence of a local PDF is
retained as a data condition rather than silently removed from the denominator.

The next evidence-building output is
\path{data/processed/audit/article_url_collection.json}. For each article, it
stores the rank, canonical article identifiers, PDF path, TEI path, HTML path,
OpenAlex metadata, and the article-related URLs extracted from the GROBID HTML
and TEI representations. At this stage the URLs are still unclassified; the
file is a normalized article-to-reference map, not a workflow-availability
judgment.

The URL collection is expanded into cached reference-page evidence under
\path{data/processed/audit/pages/}. These records add HTTP status, redirect
chains, final URLs, response headers, stored body paths, and fetch errors where
applicable. For pages that ordinary HTTP retrieval cannot access, the browser
fetch step adds rendered-page records under
\path{data/processed/audit/browser_pages/}. Those outputs distinguish ordinary
retrieval failures from browser-visible pages and challenge pages. Their role
is to make the later workflow-availability classification depend on stored
article references and stored web evidence rather than on an unrecorded live
browsing session.

Two LLM-derived data products are then used in this subsection. The first is
\path{data/processed/audit/article_reference_llm_classifications.json}. For
each cached article reference, it records a workflow-obtainability
classification, the workflow form when identifiable, whether manual checking is
required, and the page evidence used for the classification. The second is
\path{data/processed/audit/article_supplementary_llm_flags.json}. For each
article, it records supplementary boolean flags such as KNIME use, version
reporting, workflow figures, textual workflow descriptions, data availability,
code availability, and extension information, together with short evidence
snippets where available. These LLM steps use temperature 0 and bounded prompt
schemas, but the output is still treated as model-dependent derived evidence
rather than as raw observation.

The central data product for the tables is
...
into one compact record per article. Each record contains the article PDF path,
DOI, title, publication year, statistics-ready flag fields, and linked
resources restricted to confirmed, possible, or manually checked workflow
references. The report also stores summary counts used to check the generated
tables.

After workflow retrieval attempts, the workflow inventory in
\path{data/processed/audit/knime_downloadable_workflow_references.json} adds
artifact-level retrieval evidence. It records, for each workflow-relevant
article reference, the source URL, local workflow directory, downloaded files,
workflow files found, download status, and whether the successful retrieval was
automatic or manual. The article report is then updated from this inventory so
that article-level workflow-availability flags reflect workflow artifacts that
were actually obtained. Finally, the table-source CSV files under
\path{article/tables/} are generated from the compact report and workflow
inventory rather than edited manually.

Table~\ref{tab:top-cited-assessment} summarizes the statistics-ready fields in
the compact article audit report. Seventy-five records use KNIME as a
workflow, tool, interface, or implementation context. Sixty-eight records
contain textual workflow or node descriptions, and forty-nine show workflow
screenshots or figures. Twenty-three records report downloadable or linked
KNIME workflow files, and workflow artifacts were obtained for eighteen article
records after the workflow-inventory step. Version and dependency reporting are
less complete: eighteen records report a KNIME version, twenty-three report
KNIME extension or plugin dependencies, and four report an installation source.
Data and code signals are recorded separately from workflow-artifact evidence:
sixty-six records report input-data availability, twenty provide a direct
input-data resource, and twenty provide code or scripts. We do not verify the
availability of reported data or code in this table; we record whether the
article and linked evidence report those resources.
...
Reports downloadable workflows & 23 & 23.0\% & Executable workflow files provided or linked \\
...
Reports downloadable workflows & 23 & 30.7\% \\
...
confirmed successful end-to-end run, or failed during execution. Four article
records had at least one workflow execute successfully in the recorded manual
checks: PAINS, Webinar Pricing Analytics, ImageJ ecosystem integration, and
...
confirmed for four article records in the recorded manual checks, and the
