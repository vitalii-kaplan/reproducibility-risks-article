...
study, combining OpenAlex bibliometrics, an 80-record top-cited article audit
registry,
...
rather than downloadable workflow artifacts. In the 80-record article audit
...
Our top-cited article collection covers the 80 most-cited KNIME-matching
records, with not-assessed placeholders retained where local full text is
unavailable. The completed structured audit subset reported in
Table~\ref{tab:top-cited-assessment} shows that publication practice is the
weak link. In the 80-record article audit registry, twenty-two records
...
by \texttt{cited\_by\_count} in descending order and selected the 80
...
available. Not all 80 papers could be downloaded; records without local full
...
The top-cited article dataset records the structured assessment of the 80
...
\path{data/original/workflows/knime_downloadable_workflow_references.json}.
...
Table~\ref{tab:top-cited-assessment} uses all 80 records in the
...
80 records in the registry as the denominator.}
...
\bottomrule
\endfoot
Audit records & 80 & 100.0\% & Most-cited KNIME-matching article records selected from OpenAlex \\
