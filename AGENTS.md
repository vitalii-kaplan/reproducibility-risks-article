...
- `data/processed/openalex/openalex_knime_most_cited.csv` is the
  citation-ranked seed table for the expanded audit and should contain the top
  80 OpenAlex records by citation count. It is generated by
  `scripts/build_openalex_knime_bibliometrics.py` with
  `--most-cited-limit 80`, exposed in the `Makefile` as
...
- The current workflow-reference inventory covers 22 article records with
